...
So, this means that average carbon intensity simply is a weighted average of the percent of electricity that is generated by each source multiplied by the carbon intensity of the generation source. The carbon intensities for the fuel sources are `CAISO_carbon_intensity.json` `MISO_carbon_intensity.json`.
